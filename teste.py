...
def cadastrar_produto(produtos: dict, codigo: int):
    nome, preco, estoque, perecivel, descricao = ler_dados_produto()
    persistir_dados_produto(produtos, codigo, nome, preco, estoque, perecivel, descricao)
...
def ler_dados_produto():
    ...
    perecivel = ler_dado("Perecivel (0=Não, 1=(Sim) " ,bool,validar_perecivel_produto)    
    return nome, preco, estoque, perecivel ,descricao
...
def validar_descricao_produto(descricao: str) -> bool:
    if len(descricao) < 12:
        print("A descrição do produto deve conter 12 ou mais caracteres. Tente novamente.")
        return False
    else:
        return True
...
def persistir_dados_produto(produtos, codigo, nome, preco, estoque, perecivel, descricao):
    produtos[codigo] = (nome, preco, estoque, perecivel, descricao )
